"""The ball and the second prop through the mash: class switches, velocity and impulses, per vsync.

    python tools/props.py

Records, for the ball (01ADD9D0) and the second prop (01A94440), the class
pointer (+0x0C), velocity (+0x20..+0x2C), position (+0x540..) and +0x120..+0x124,
through 130 vsyncs of the mash at 30fps and at 60fps + [60 FPS - ball physics].
Prints every class change and every impulse (vy jumping up by more than 5, or
horizontal speed jumping by more than 5). Writes work/props.npz.

2026-09-15: both props launch at the same vsync in both arms (the ball at 26/27
into class 01C60340, prop 2 at 51 into class 01C60040), but at 60fps the ball
took weak -10 hits at vsyncs 67 and 91 and a side swipe at 116 that 30fps never
got.
"""
...
def main() -> int:
    ...
    for key, fps, words in (("o30", 30, None), ("f60", 60, fix)):
        ...
        for t in range(N):
            ...
            for k, obj in enumerate((sandlot.BALL, sandlot.PROP2)):
        ...
        for k, label in enumerate(("ball", "prop2")):
