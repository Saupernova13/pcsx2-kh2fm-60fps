...
it was. This project's groups are inserted next to the groups they depend on, and
any earlier copy of them is replaced.
...
from game.pnachtext import block_range, insert_after, normalise_blank_lines, patch_words, remove_block
from ps2ee.pnach import Pnach

BALL = "60 FPS - ball physics"
WIDESCREEN = "Widescreen 19.5:9 - S24 Ultra"
OURS = [BALL, WIDESCREEN]


# --- this project's groups ---------------------------------------------------

def ball_block() -> list[str]:
    path = config.PATCHES / "kh2fm-60fps.pnach"
    lines = path.read_text(encoding="utf-8").splitlines()
    found = block_range(lines, BALL)
    if not found:
        raise SystemExit(f"[{BALL}] is missing from {path}")
    start, _, end = found
...
def status() -> int:
    ...
    for name in ["60 FPS", "Widescreen 16:9"] + OURS:
        found = block_range(lines, name)
        mark = "present" if found else "absent "
        print(f"  [{name}]  {mark}  {'enabled' if name in enabled else 'off'}")
    found = block_range(lines, BALL)
    if found:
        same = patch_words(lines[found[0]:found[2]]) == patch_words(ball_block())
        print(f"  [{BALL}] words match this repo: {same}")
...
def main() -> int:
    ...
    text, source = base_text()
    lines = text.splitlines()
    before = {name: (patch_words(lines[r[0]:r[2]]) if (r := block_range(lines, name)) else None) for name in OURS}
    ...
    if args.remove:
        ...
        if not block_range(lines, identity.REQUIRES[BALL]):
            raise SystemExit(f"[{identity.REQUIRES[BALL]}] is not in {source}; [{BALL}] needs it.")
        block = ball_block()
        validate(block, BALL)
        insert_after(lines, identity.REQUIRES[BALL], block)
        enable.append(BALL)
        if args.enable_60fps:
            enable.append(identity.REQUIRES[BALL])
        if args.widescreen:
    ...
    if identity.REQUIRES[BALL] not in current and identity.REQUIRES[BALL] not in enable and not args.remove:
        print(f"  NOTE [{identity.REQUIRES[BALL]}] is not enabled, so [{BALL}] will do nothing. Pass --enable-60fps.")
    ...
    new_ini = edit_ini(ini_text, enable, disable).replace("\n", ini_nl) if ini_nl != "\n" else edit_ini(ini_text, enable, disable)
